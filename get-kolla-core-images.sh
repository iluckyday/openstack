...
cat etc_kolla_globals.yml | sed "s/LATEST_RELEASE/${LATEST_RELEASE}/" | sed '/openstack_core/,$d' | tee /etc/kolla/globals.yml
...
docker save $(docker image list "kolla/${DISTRO}-${TYPE}-*" -q) | xz > /tmp/dockerhub-kolla-core-${DISTRO}-${TYPE}-images-${LATEST_RELEASE}.tar.xz

echo dockerhub-kolla-core-${DISTRO}-${TYPE}-images-${LATEST_RELEASE} is:
ls -lh /tmp/dockerhub-kolla-core-${DISTRO}-${TYPE}-images-${LATEST_RELEASE}.tar.xz
